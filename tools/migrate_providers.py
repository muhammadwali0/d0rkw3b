...
def build():
    ...
    for module in MODULES:
        source = subprocess.check_output(
            ["git", "show", f"9e49913:{module}.py"], cwd=ROOT, text=True
        )
        ...
        tree.body = [
            ...
            if not (
                ...
                and any(a.name == "requests" for a in node.names)
            )
        ]
        ...
        for name, func in scope.items():
            ...
            for path, raw in leaves(func(*(markers[v] for v in variables))):
                identifier = re.sub(
                    "[^a-z0-9]+", "-", "-".join((module, name, *path)).lower()
                ).strip("-")
                ...
                notes = [
                    "Migrated from legacy code; endpoint availability has not been verified."
                ]
                enabled = True
                ...
                if needs_key:
                    enabled = False
                    notes.append(
                        "Requires an owner-supplied API key; bundled credential removed. Endpoint retained for reference only."
                    )
                if "graphql/query/" in raw:
                    enabled = False
                    notes.append(
                        "Undocumented authenticated GraphQL query hash; retained for reference only."
                    )
                if any(
                    token in raw
                    for token in ("/portscan/", "/traceroute/", "ssltest/analyze")
                ):
                    enabled = False
                    notes.append(
                        "Visiting this launcher can initiate a remote probe; disabled by default."
                    )
                if "wigle.net" in raw:
                    enabled = False
                    notes.append(
                        "Legacy IP input was used as an SSID or postal code; questionable mapping."
                    )
                if (
                    module == "images"
                    and path[-1] == "Google Images"
                    and "image_keyword" in name
                ):
                ...
                if name == "x_username_by_year":
                    template = template.replace(
                        "until%3A{year}-12-31", "until%3A{next_year}-01-01"
                    )
                    notes.append(
                        "Exclusive end date corrected to January 1 of the following year."
                    )
                target = variables[0]
                ...
                network = (
                    "tor"
                    if (urlsplit(raw).hostname or "").endswith(".onion")
                    else "clearnet"
                )
                auth = "unknown"
                if any(
                ...
                record = dict(
                    ...
                    target_types=types,
                    category=module,
                    description=f"{module}: {' / '.join(path)}",
                    template=template,
                    tags=[name],
                    network=network,
                    auth=auth,
                    requires_api_key=needs_key,
                    homepage=f"{urlsplit(raw).scheme}://{urlsplit(raw).netloc}/".replace(
                        "D0RKMARKERUSERNAMEEND.", ""
                    ),
                    enabled=enabled,
                    status="unverified",
                    last_verified=None,
                    notes=" ".join(notes),
                    source=f"{module}.py:{name}",
    ...
    (ROOT / "docs/migration-inventory.json").write_text(
        json.dumps(inventory, indent=2) + "\n"
    )
    for module in MODULES:
        (ROOT / f"src/d0rkw3b/providers/{module}.json").write_text(
            json.dumps([p for p in providers if p["category"] == module], indent=2)
            + "\n"
        )
    print(
        f"Inventoried {len(providers)} URLs from {len(set(p['source'] for p in providers))} functions across {len(MODULES)} modules."
    )
