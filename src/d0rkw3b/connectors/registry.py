...
def discover_plugins():
    ...
    for entry in entry_points(group="d0rkw3b.connectors"):
        try:
            if (
                entry.name in MANIFESTS
                or entry.name == "exiftool"
                or entry.name in plugins
            ):
                raise ValueError("duplicate or reserved connector ID")
            distribution = entry.dist
            paths = [
                p for p in (distribution.files or ()) if p.name == "d0rkw3b-plugin.json"
            ]
            if len(paths) != 1:
                raise ValueError(
                    "plugin distribution must include exactly one d0rkw3b-plugin.json"
                )
            ...
            if (
                set(document) != {"api_version", "connectors"}
                or document["api_version"] != 1
            ):
            ...
            if (
                not isinstance(capabilities.network, list)
                or any(not isinstance(v, str) or not v for v in capabilities.network)
                or type(capabilities.filesystem_read) is not bool
                or type(capabilities.subprocess) is not bool
            ):
                raise ValueError("invalid plugin capability declarations")
            capabilities = Capabilities(
                tuple(capabilities.network),
...
def collect(entity, connector_id, *, disclose=None, timeout=None):
    ...
    if connector_id in load_settings()["disabled_connectors"]:
        raise ConnectorPermissionError(
            "connector is disabled by local configuration: " + connector_id
        )
    plugins = {}
    if connector_id not in MANIFESTS and connector_id != "exiftool":
        ...
        if connector_id not in plugins:
            detail = (
                "; ".join(errors) if errors else "use connectors to list available IDs"
            )
            raise ConnectorUnavailableError(
                f"connector unavailable: {connector_id}; {detail}"
            )
    ...
    if missing:
        raise ConnectorPermissionError(
            "required credential environment variables are absent: "
            + ", ".join(missing)
        )
    limit = manifest.timeout_seconds if timeout is None else timeout
    if not isinstance(limit, (int, float)) or not 0 < limit <= 60:
        raise ValueError(
            "connector timeout must be greater than zero and at most 60 seconds"
        )
    ...
    if connector_id == "exiftool":
        ...
        try:
            ...
            raise ConnectorUnavailableError(
                f"plugin {connector_id} could not load ({type(exc).__name__})"
            ) from exc
    if connector_id in plugins:
        try:
            ...
            raise ConnectorUnavailableError(
                f"plugin {connector_id} failed ({type(exc).__name__}); its message is suppressed to avoid leaking secrets"
            ) from exc
    else:
        result = runner(entity, timeout=limit)
    if (
        not isinstance(result, Collection)
        or result.entity.entity_id != entity.entity_id
    ):
        raise ConnectorUnavailableError(
            "connector returned an invalid collection or changed the input entity"
        )
