    ...
    with tempfile.TemporaryFile() as stdout, tempfile.TemporaryFile() as stderr:
        try:
            ...
            raise ConnectorUnavailableError(
                "external executable could not start"
            ) from exc
        ...
        try:
            while True:
                if (
                    ...
                    raise ConnectorUnavailableError(
                        "external output exceeded the size limit"
                    )
            ...
            if code:
                raise ConnectorUnavailableError(
                    f"external tool exited with status {code}; no result accepted"
                )
            stdout.seek(0)
            ...
            raise ConnectorUnavailableError(
                "external tool returned malformed JSON"
            ) from exc
        finally:
