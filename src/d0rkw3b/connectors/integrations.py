...
EXIFTOOL = Manifest('exiftool', 'ExifTool local metadata', ('file',), (),
    ...
    'One regular local file; fixed read-only options, 2 MiB output cap, no user ExifTool config.', 15)

TOOLS = {'sherlock': 'sherlock', 'maigret': 'maigret', 'exiftool': 'exiftool',
         'theharvester': 'theHarvester', 'amass': 'amass'}


def integrations():
    return [{'id': key, 'executable': shutil.which(binary),
...
def exiftool(entity, *, timeout=15):
    ...
    if executable is None:
        raise ConnectorUnavailableError('ExifTool is not installed; install it separately to enable this optional adapter')
    path, stream = open_regular(entity.normalized_value)
    stream.close()
    tags = ['-FileType', '-MIMEType', '-ImageWidth', '-ImageHeight', '-Make', '-Model',
            '-DateTimeOriginal', '-CreateDate', '-ModifyDate', '-GPSLatitude', '-GPSLongitude',
            '-GPSAltitude', '-Author', '-Title', '-Creator', '-Producer', '-PageCount']
    rows = run_json([executable, '-config', '', '-j', '-G1', '-n', '-s', *tags, '--', str(path)], timeout=timeout)
