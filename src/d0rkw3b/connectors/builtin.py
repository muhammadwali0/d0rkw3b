...
MANIFESTS = {
    "dns": Manifest(
        ...
        "One resolver lookup per invocation; OS DNS caching/retries apply.",
        10,
    ),
    "rdap": Manifest(
        "rdap",
        ...
        Capabilities(
            network=(
                "data.iana.org",
                "HTTPS RDAP registry selected from IANA bootstrap",
            )
        ),
        "Read public registration metadata; domain nameservers are registry observations, not ownership attribution.",
        "Two bounded GET requests; no redirects or retries; 429/403 require manual follow-up.",
        10,
    ),
    "local-file": Manifest(
        "local-file",
        "Local file metadata and SHA-256",
        ("file",),
        ("file_hash",),
        Capabilities(filesystem_read=True),
        "Hash a regular local file and inspect bounded header/text metadata; no upload.",
        "One file streamed locally; parsing uses the first 1 MiB. Runtime scales with file size.",
        10,
    ),
}
...
def observation(
    ...
    *,
    ...
    return Observation(
        identifier("observation", ref, metadata),
        "observed",
        source,
        method,
        entity.entity_id,
...
def dns(entity, *, timeout=10):
    ...
    values = run_json(
        [sys.executable, "-I", "-c", script, entity.normalized_value], timeout=timeout
    )
    if (
    ...
    for value in sorted(set(values)):
        ...
        provenance = Provenance(
            "dns",
            "OS getaddrinfo A/AAAA",
            identifier("dns", entity.entity_id, value, collected),
            collected,
            notes="Resolver answer at collection time; not a claim of ownership or DNSSEC validation.",
        )
        relation = Relationship.create(
            entity, "resolves_to", output, provenance=provenance
        )
        entities.append(output)
        relations.append(relation)
        observations.append(
            Observation(
                identifier("observation", provenance.reference),
                "observed",
                "dns",
                provenance.method,
                entity.entity_id,
...
def rdap_base(bootstrap, entity):
    ...
    for service in services:
        ...
        for suffix in suffixes:
            ...
            if entity.type == "domain":
                match = (
                    entity.normalized_value == suffix
                    or entity.normalized_value.endswith("." + suffix)
                )
    ...
    if not candidates:
        raise ConnectorUnavailableError(
            "no matching HTTPS RDAP service in IANA bootstrap"
        )
    return (
        sorted(candidates, key=lambda item: (-item[0], item[1]))[0][1].rstrip("/") + "/"
    )
...
def get_json(url, timeout):
    ...
    if status != 200:
        raise ConnectorUnavailableError(
            f"RDAP request returned HTTP {status}; no redirect, retry or bypass attempted"
        )
    try:
...
def rdap(entity, *, timeout=10):
    ...
    endpoint = (
        base
        + ("domain/" if entity.type == "domain" else "ip/")
        + quote(entity.normalized_value, safe="")
    )
    data = get_json(endpoint, timeout)
    expected = "domain" if entity.type == "domain" else "ip network"
    if data.get("objectClassName") != expected:
        raise ConnectorUnavailableError(
            "RDAP response does not match the requested object class"
        )
    ...
    if entity.type == "domain":
        ...
        if (
            ...
            or Entity.create(name, "domain").entity_id != entity.entity_id
        ):
        ...
        try:
            ...
            if (
                not ipaddress.ip_address(data["startAddress"])
                <= address
            ...
            raise ConnectorUnavailableError(
                "RDAP response does not cover the requested IP"
            ) from exc
    collected = now()
    ...
    if entity.type == "domain":
        ...
        for ns in nameservers[:100]:
            ...
            relation = Relationship.create(
                entity, "uses_nameserver", output, provenance=provenance
            )
            entities.append(output)
            relations.append(relation)
...
def local_file(entity, *, timeout=10):
    ...
    collected = now()
    output = Entity.create(
        "sha256:" + data["sha256"], "file_hash", created_at=collected
    )
    provenance = Provenance(
        "local-file",
        "streamed SHA-256",
    ...
    observed = Observation(
        identifier("observation", provenance.reference, collected),
        "observed",
        "local-file",
        provenance.method,
        entity.entity_id,
