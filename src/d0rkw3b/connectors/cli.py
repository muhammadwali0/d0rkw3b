...
def disclosure(manifest, entity=None):
    ...
    print('Required credential variables: ' + (', '.join(manifest.required_keys) or 'none'), file=sys.stderr)
    print('Capabilities: ' + json.dumps(manifest.to_dict()['capabilities']), file=sys.stderr)
...
def dispatch(argv):
    if not argv or argv[0] not in ('collect', 'connectors', 'integrations', 'file'):
        return None
    parser = argparse.ArgumentParser(prog='d0rkw3b ' + argv[0], description='Explicit collection and optional installed tools. No background collection.')
    parser.add_argument('--format', choices=('text', 'json'), default='json')
    if argv[0] in ('collect', 'file'):
        parser.add_argument('target', nargs='+', help='[type] target, or local file path')
        parser.add_argument('--type')
        parser.add_argument('--connector', required=argv[0] == 'collect', default='local-file')
        parser.add_argument('--with', dest='additional', action='append', default=[], help='explicit additional connector, e.g. exiftool')
        ...
        if argv[0] == 'integrations':
            ...
            if args.case:
                with CaseStore() as store:
                    store.case(args.case)  # Fail before any collection if the destination case is absent.
            ...
            results = [collect(entity, key, disclose=disclosure, timeout=args.timeout) for key in ids]
            if args.case:
                with CaseStore(writable=True) as store:
                    for key, item in zip(ids, results):
                        store.save(args.case, item, method='collect:' + key)
            result = results[0].to_dict() if len(results) == 1 else [r.to_dict() for r in results]
