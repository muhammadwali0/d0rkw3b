...
def parser():
    ...
    result.add_argument(
        "target",
        nargs="*",
        help="[type] target, or interactive / providers / validate-providers",
    )
    result.add_argument("--verbose", action="store_true", help="diagnostics on stderr")
    result.add_argument(
        "--debug",
        action="store_true",
        help="traceback frames on stderr for errors; no locals or chained exceptions",
    )
    ...
    result.add_argument(
        ...
        action="append",
        default=[],
        help="select exact provider ID; repeatable",
    )
    result.add_argument(
        "--provider-file",
        action="append",
        default=[],
        help="additional JSON provider file or directory",
    )
    result.add_argument(
        "--network",
        choices=("clearnet", "tor", "all"),
        default="clearnet",
        help="default: clearnet; Tor links are never automatically opened",
    )
    result.add_argument(
        "--include-disabled",
        action="store_true",
        help="include retained reference definitions",
    )
    result.add_argument(
        "--param",
        action="append",
        default=[],
        metavar="NAME=VALUE",
        help="extra parameter, e.g. year=2024, query=term, username2=other",
    )
    result.add_argument(
        "--format",
        choices=("text", "json", "csv", "markdown"),
        default=load_settings()["default_format"],
    )
    result.add_argument(
        "--all",
        action="store_true",
        help="show every matching provider (text defaults to top 10; machine formats remain complete)",
    )
    result.add_argument(
        "--open",
        action="store_true",
        help="open one explicitly selected clearnet provider in your browser; sends target to provider",
    )
    return result
...
def output(rows, fmt):
    ...
    if fmt == "csv":
        ...
        writer = csv.DictWriter(
            stream, fieldnames=["id", "name", "category", "network", "status", "url"]
        )
        writer.writeheader()
    ...
    return "".join(
        f"{row['id']} [{row['status']}, {row['network']}]\n  {row['url']}\n"
        for row in rows
    )
...
def menu_query(category, providers):
    ...
    if any(p["network"] == "tor" for p in entries):
        network = (
            input("Network (clearnet/tor/all; default clearnet): ").strip()
            or "clearnet"
        )
    ...
    try:
        if args.target == ["interactive"]:
            if not sys.stdin.isatty():
                cli.error(
                    "interactive requires a terminal; use direct commands for scripts"
                )
            return interactive()
        ...
        if args.target == ["validate-providers"]:
            if args.format == "json":
                print(
                    json.dumps(
                        {"providers": len(providers), "errors": errors}, indent=2
                    )
                )
            else:
        ...
        providers = [
            ...
            if (not selected or p["id"] in selected)
            ...
            and (
                ...
                or (
                    p["enabled"]
                    and p["status"] not in ("disabled", "broken", "deprecated")
                )
            )
        ]
        if args.target == ["providers"]:
            if args.format == "json":
                ...
                print(
                    output(
                        [dict(p, url=p["template"]) for p in providers], args.format
                    ),
                    end="",
                )
            return 0
        ...
        for parameter in args.param:
            ...
            if (
                not separator
                or key not in VARIABLES - {"next_year", target.variable}
                or key in extras
            ):
        ...
        for provider in providers:
            ...
            rows.append(
                {
                    k: provider[k]
                    for k in ("id", "name", "category", "network", "status")
                }
                | {"url": url}
            )
        if not rows:
            raise ValueError(
                "no matching providers; check type, filters and required parameters"
            )
        if args.open:
            if len(selected) != 1 or len(rows) != 1:
                raise ValueError(
                    "--open requires exactly one --provider and one matching link"
                )
            provider = next(p for p in providers if p["id"] == rows[0]["id"])
            if provider["network"] == "tor":
                raise ValueError(
                    "Tor links must be copied into a separately configured Tor Browser; automatic opening is disabled"
                )
            if (
                not provider["enabled"]
                or provider["requires_api_key"]
                or provider["status"] in ("broken", "disabled", "deprecated")
            ):
                raise ValueError(
                    "disabled/reference providers cannot be automatically opened"
                )
        ...
        if args.format == "text" and not args.all and not selected and len(rows) > 10:
            print(
                f"{len(rows) - 10} additional providers available with --all",
                file=sys.stderr,
            )
            rows = rows[:10]
...
def main(argv=None):
    ...
    try:
        if verbose:
            print(
                "D0RKW3B: command dispatch; targets and credentials are omitted from diagnostic logs.",
                file=sys.stderr,
            )
        return _main(argv)
    except BrokenPipeError:
