...
def dispatch(argv):
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "Reachability is not semantic verification."
            if health
            else "Offline registry counts."
        ),
    )
    ...
    if health:
        ...
        parser.add_argument(
            ...
            type=int,
            default=10,
            help="unique hosts, 1–100 (default 10)",
        )
        parser.add_argument(
            "--timeout",
            type=float,
            default=5,
            help="socket timeout, seconds; OS controls DNS timeout",
        )
        parser.add_argument(
            "--delay",
            type=float,
            default=0.5,
            help="seconds between requests, minimum 0.25",
        )
    ...
    try:
        ...
        if health:
            ...
            if args.format == "json":
                ...
                print(
                    f"{result['request_count']} request attempts; no semantic verification performed"
                )
                for row in result["results"]:
                    print(
                        f"{row['id']}: {row['health']} (semantic: {row['semantic_status']})"
                    )
        else:
            result = {
                "total": len(providers),
                ...
                **{
                ...
                "target_types": dict(
                    sorted(
                        Counter(t for p in providers for t in p["target_types"]).items()
                    )
                ),
            }
            print(json.dumps(result, indent=2))
