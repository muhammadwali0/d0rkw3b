...
from .core.validation import domain_name
...
def inspect_stream(stream, filename, *, copy_to=None, timeout=None):
    ...
    metadata = {
        ...
        "filesystem_ctime": datetime.fromtimestamp(
            before.st_ctime, timezone.utc
        ).isoformat(),
    ...
    if (
        ...
        and len(sample) >= 24
        and sample[12:16] == b"IHDR"
    ):
        metadata.update(
            mime_type="image/png",
            mime_basis="PNG signature",
            dimensions=list(struct.unpack(">II", sample[16:24])),
        )
    elif sample[:6] in (b"GIF87a", b"GIF89a") and len(sample) >= 10:
        metadata.update(
            mime_type="image/gif",
            mime_basis="GIF signature",
            dimensions=list(struct.unpack("<HH", sample[6:10])),
        )
    ...
    if b"\x00" not in sample:
        try:
            ...
            metadata["references"] = {
                "urls": sorted(
                    set(re.findall(r'https?://[^\s<>"\x00-\x1f]{1,2048}', text))
                )[:100],
                "emails": sorted(
                    set(
                        re.findall(r"[\w.+-]{1,64}@[\w.-]{1,253}\.[A-Za-z]{2,63}", text)
                    )
                )[:100],
            }
            candidates = set()
            for token in re.findall(r"[A-Za-z0-9.-]{3,253}", text):
                if "." not in token:
                    continue
                try:
                    candidates.add(domain_name(token.strip(".")))
                except ValueError:
                    pass
                if len(candidates) >= 100:
                    break
            metadata["references"]["domains"] = sorted(candidates)
            metadata["references_scope"] = (
                "first 1 MiB of UTF-8 text; candidates, not verified entities"
            )
    return metadata
