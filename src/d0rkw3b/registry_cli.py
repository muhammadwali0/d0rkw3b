...
def dispatch(argv):
    if argv[:2] == ['packs', 'list']:
        parser = argparse.ArgumentParser(prog='d0rkw3b packs list', description='Logical groups of existing query providers.')
        ...
        if args.format == 'json':
            ...
            for pack in result:
                print(f'{pack["id"]}: {pack["providers"]} providers — {pack["description"]}')
        return 0
    ...
    if argv[1] == 'update':
        ...
        parser.add_argument('--sha256', required=True, help='expected SHA-256 of the input file, obtained independently')
    args = parser.parse_args(argv[2:])
    try:
        bundled, errors = load_bundled_registry()
        if errors:
            raise ValueError('bundled registry has validation errors')
        result = install_snapshot(args.file, args.sha256, bundled) if argv[1] == 'update' else rollback(bundled)
        print(json.dumps(result, indent=2))
        return 0
