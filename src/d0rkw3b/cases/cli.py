...
def dispatch(argv):
    ...
    parser = argparse.ArgumentParser(
        prog="d0rkw3b " + argv[0],
        description="Explicit local case and evidence storage. No network access.",
    )
    parser.add_argument(
        "action",
        choices=(
            ("new", "list", "show", "add", "note", "export", "graph", "timeline")
            if argv[0] == "case"
            else ("add",)
        ),
    )
    ...
    parser.add_argument(
        "--format",
        choices=("text", "json", "jsonl", "csv", "markdown", "graphml", "gexf"),
        default=None,
    )
    args = parser.parse_args(argv[1:])
    args.format = args.format or ("graphml" if args.action == "graph" else "json")
    if args.action == "graph" and args.format not in ("graphml", "gexf"):
        parser.error("case graph supports graphml or gexf")
    ...
    try:
        with CaseStore(writable=args.action in ("new", "add", "note")) as store:
            if argv[0] == "evidence":
                ...
                run = store.save(
                    args.name,
                    investigate(args.value, type=args.type),
                    method="case add",
                )
                ...
                if args.action == "graph":
                    print(
                        graph(
                            result,
                            args.format
                            if args.format in ("graphml", "gexf")
                            else "graphml",
                        ),
                        end="",
                    )
            ...
            if args.format == "json":
                ...
                print(
                    export_data(
                        [{"record_type": args.action, "record": row} for row in rows],
                        args.format,
                    ),
                    end="",
                )
