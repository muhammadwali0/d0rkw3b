...
class CaseStore:
    def __init__(self, root=None, *, writable=False):
        ...
        try:
            ...
            self.db = sqlite3.connect(
                self.path.as_uri() + "?mode=" + mode, uri=True, timeout=5
            )
            ...
            if version == 0:
                if not writable:
                    raise StorageError(
                        "uninitialized database; use an explicit case write command"
                    )
                if self.db.execute(
                    "SELECT name FROM sqlite_master WHERE type='table'"
                ).fetchone():
                    raise StorageError(
                        "refusing to initialize an unrecognized nonempty database"
                    )
    ...
    def list(self):
        ...
        return [
            ...
            for row in self.db.execute(
                "SELECT case_id, name, created_at FROM cases ORDER BY name"
            )
        ]
    ...
    def case(self, name):
        if self.db is not None:
            row = self.db.execute(
                "SELECT case_id, name, created_at FROM cases WHERE name=?", (name,)
            ).fetchone()
            if row:
                return dict(row)
    ...
    def new(self, name):
        ...
        with self.db:
            try:
                self.db.execute(
                    "INSERT INTO cases VALUES (?,?,?)", (uuid.uuid4().hex, name, now())
                )
            ...
            self.db.execute(
                ...
                (
                    ...
                    encode(
                        {
                            "method": method,
                            "raw_input": result.entity.raw_value,
    ...
    def show(self, name):
        ...
        for table, order in [
            ("entities", "entity_id"),
            ("runs", "created_at,run_id"),
            ("relationships", "run_id,relationship_id"),
            ("observations", "timestamp,run_id,observation_id"),
            ("notes", "created_at,note_id"),
            ("evidence", "imported_at,evidence_id"),
        ]:
            rows = []
            # Table and ordering are fixed application constants, never user input.
            for row in self.db.execute(
                f"SELECT * FROM {table} WHERE case_id=? ORDER BY {order}",
                (case["case_id"],),
            ):
                value = dict(row)
