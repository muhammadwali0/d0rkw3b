...
def investigate(value, *, type=None, recipe=None, all=False, network='clearnet',
    ...
    if stages is None:
        ...
        stages = [{'name': category, 'description': 'Generated query paths; manually assess returned information.',
    ...
    for stage in stages:
        ...
        for provider in available:
            if provider['id'] not in stage['providers']:
                continue
            needed = fields(provider['template']) - {target.variable} - (parameters or {}).keys()
            if 'year' in (parameters or {}):
                needed.discard('next_year')
            if needed:
                diagnostics.append(f'{provider["id"]}: requires {", ".join(sorted(needed))}')
                continue
            try:
                ...
                continue
            queries.append({'id': provider['id'], 'name': provider['name'], 'url': url,
                            'kind': 'generated_query', 'network': provider['network'],
                            'status': provider['status'], 'ranking': ranking(provider)})
        total = len(queries)
        queries = queries if all else queries[:5]
        for query in queries:
            if query['id'] not in seen:
                provenance = Provenance(query['id'], 'template rendering', query['url'], entity.created_at,
                                        source_url=query['url'], notes='Generated locally; destination not contacted.')
                observations.append(Observation(identifier('observation', entity.entity_id, query['id'], query['url']),
                    'generated_query', query['id'], 'template rendering', entity.entity_id, entity.created_at,
                    provenance, metadata={'url': query['url']}))
                seen.add(query['id'])
        if total:
            paths.append({'name': stage['name'], 'description': stage['description'],
                          'queries': queries, 'additional': total - len(queries)})
    entities, relationships = [entity], []
    if entity.type == 'email':
        domain = Entity.create(entity.normalized_value.rsplit('@', 1)[1], 'domain', created_at=entity.created_at)
        provenance = Provenance('input', 'email syntax parsing', entity.entity_id, entity.created_at,
                                notes='Syntactic domain component only; no mailbox or DNS verification.')
        relationship = Relationship.create(entity, 'uses_domain', domain, provenance=provenance)
        entities.append(domain)
        relationships.append(relationship)
    return Investigation(entity, entities, relationships, observations, paths, diagnostics, recipe)
