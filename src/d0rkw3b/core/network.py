...
def request(url, *, method='HEAD', timeout=5, max_bytes=1_048_576):
    ...
    if (parts.scheme not in ('http', 'https') or not host or
            ...
            host.lower().rstrip('.').endswith('.onion') or
            any(c.isspace() or ord(c) < 32 for c in url)):
        raise NetworkPolicyError('only public HTTP(S) destinations without credentials are allowed')
    ...
    try:
        ...
        if parts.scheme == 'https':
            transport = ssl.create_default_context().wrap_socket(transport, server_hostname=host)
