...
def validate_snapshot(providers, bundled):
    ...
    for original in bundled:
        ...
        if updated is None:
            raise ValueError(
                "snapshot would remove bundled provider: " + original["id"]
            )
        if (
            not set(original["target_types"]).issubset(updated["target_types"])
            or original["category"] != updated["category"]
            or original["network"] != updated["network"]
        ):
            raise ValueError(
                "snapshot would reduce or change bundled coverage: " + original["id"]
            )
    return providers
...
def snapshot_path(root=None):
    return (
        Path(root if root is not None else data_directory())
        / "registry"
        / "current.json"
    )
...
def read_snapshot(path, bundled):
    ...
    if (
        ...
        or set(envelope) != {"schema_version", "sha256", "providers"}
...
def install_snapshot(source, expected_sha256, bundled, root=None):
    ...
    if (
        len(raw) > MAX_BYTES
        or hashlib.sha256(raw).hexdigest() != expected_sha256.lower()
    ):
    ...
    envelope = {
        "schema_version": 1,
        "sha256": hashlib.sha256(canonical(providers)).hexdigest(),
        "providers": providers,
    }
