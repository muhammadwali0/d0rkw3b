...
def validate_provider(item):
    ...
    if item.keys() - REQUIRED - OPTIONAL:
        raise ValueError(
            "unknown fields: " + ", ".join(sorted(item.keys() - REQUIRED - OPTIONAL))
        )
    for key in (
        "id",
        "name",
    ...
    for key, allowed in (("target_types", TARGET_TYPES), ("parameters", VARIABLES)):
        if (
            ...
            or not item[key]
            or any(not isinstance(v, str) or v not in allowed for v in item[key])
        ):
            raise ValueError(f"invalid {key}")
    if not isinstance(item["tags"], list) or any(
        not isinstance(v, str) for v in item["tags"]
    ):
    ...
    for key, allowed in (
        ...
        ("auth", {"none", "unknown", "may_require", "required"}),
    ...
    if quality is not None:
        if (
            ...
            or set(quality) != {"priority", "reason"}
            or type(quality["priority"]) is not int
            or not 0 <= quality["priority"] <= 5
            or not isinstance(quality["reason"], str)
            or not quality["reason"].strip()
        ):
            raise ValueError(
                "quality requires priority 0–5 and a nonempty editorial reason"
            )
    if "packs" in item and (
        not isinstance(item["packs"], list)
        or any(
            not isinstance(v, str) or not re.fullmatch("[a-z0-9-]+", v)
            for v in item["packs"]
        )
    ):
    ...
    if "verification" in item:
        ...
        if (
            ...
            or set(verification) != {"method", "confidence", "notes"}
            or any(
                not isinstance(v, str) or not v.strip() for v in verification.values()
            )
            or verification["confidence"] not in ("low", "medium", "high")
        ):
            raise ValueError(
                "verification requires method, confidence (low/medium/high), and notes"
            )
    ...
    if used - VARIABLES:
        raise ValueError(
            "unsupported variables: " + ", ".join(sorted(used - VARIABLES))
        )
...
def load_bundled_registry(extra_paths=()):
    ...
    sources = sorted(
        files("d0rkw3b").joinpath("providers").iterdir(), key=lambda p: p.name
    )
...
def load_registry(extra_paths=()):
    ...
    providers = [
        dict(p, enabled=False) if p["id"] in disabled else p for p in providers
    ]
    return sorted(providers, key=lambda p: p["id"]), errors
