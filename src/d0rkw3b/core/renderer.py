...
def render(provider, target, extras=None):
    ...
    if "year" in values:
        if (
            not re.fullmatch(r"[0-9]{4}", values["year"])
            or not 1 <= int(values["year"]) <= 9998
        ):
    ...
    for field in fields(host_template):
        if not re.fullmatch(
            r"[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?", values[field]
        ):
            raise ValueError(f"{field} is not a valid hostname label for this provider")
    return provider["template"].format_map(
        {k: quote(v, safe="") for k, v in values.items()}
    )
