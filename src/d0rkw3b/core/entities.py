...
class Entity:
    ...
    def __post_init__(self):
        ...
        if target.value != self.normalized_value or self.entity_id != identifier('entity', self.type, self.normalized_value):
    ...
    def create(cls, value, kind=None, *, created_at=None, metadata=None):
        ...
        return cls(identifier('entity', target.type, target.value), target.type, value,
                   target.value, created_at or now(), dict(metadata or {}))
...
RELATIONSHIPS = frozenset(('uses_domain', 'resolves_to', 'uses_nameserver', 'belongs_to',
                          'has_profile', 'owns', 'references', 'has_hash'))
...
class Relationship:
    ...
    @classmethod
    def create(cls, subject, predicate, object_, *, provenance, kind='observed',
               confidence='high', observed_at=None):
        return cls(identifier('relationship', subject.entity_id, predicate, object_.entity_id,
                              provenance.reference, kind), subject.entity_id, predicate,
                   object_.entity_id, kind, observed_at or provenance.collected_at, confidence, provenance)
...
class Observation:
    ...
    def __post_init__(self):
        ...
        if self.kind not in ('generated_query', 'observed', 'imported', 'manually-added'):
            raise ValueError('unsupported observation kind')
        if not isinstance(self.provenance, Provenance):
            raise ValueError('observation requires provenance')
        if self.kind == 'generated_query' and (self.confidence != 'unverified' or self.output_entity_id or self.relationship_id):
