...
def validate_recipe(recipe, providers):
    ...
    if not isinstance(recipe, dict) or set(recipe) != required:
        raise ValueError(
            "recipe requires exactly id, name, description, target_types and stages"
        )
    ...
    if (
        ...
        or not types
        or any(not isinstance(t, str) or t not in TARGET_TYPES for t in types)
    ):
    ...
    for stage in recipe["stages"]:
        ...
        if any(
            not isinstance(stage[k], str) or not stage[k].strip()
        ...
        for key in stage["providers"]:
            ...
            if not set(types).issubset(registry[key]["target_types"]):
                raise ValueError(
                    f"provider {key} does not accept every recipe target type"
                )
    return recipe
...
def load_recipes(providers, extra_paths=()):
    sources = sorted(
        files("d0rkw3b").joinpath("recipes").iterdir(), key=lambda p: p.name
    )
    ...
    for source in sources:
        ...
        try:
            recipe = validate_recipe(
                json.loads(source.read_text(encoding="utf-8")), providers
            )
