...
OUTCOMES = frozenset(('healthy', 'degraded', 'auth-required', 'rate-limited',
    'redirect', 'unreachable', 'dns-failure', 'tls-failure', 'timeout',
    'manual-verification-required', 'tor-unchecked', 'deprecated', 'unknown'))
...
def check_providers(providers, *, max_requests=10, timeout=5, delay=0.5, probe=request,
                    sleep=time.sleep):
    ...
    if not 1 <= max_requests <= 100 or not 0 < timeout <= 30 or not 0.25 <= delay <= 60:
        raise ValueError('use 1–100 requests, timeout >0 and <=30 seconds, delay 0.25–60 seconds')
    results, cache, requests = [], {}, 0
    for provider in sorted(providers, key=lambda p: str(p.get('id', ''))):
        row = {'id': provider.get('id'), 'structurally_valid': True, 'template_valid': True,
               'reachable': None, 'health': 'unknown', 'semantic_status': 'unverified',
               'last_checked': None, 'last_verified': provider.get('last_verified'),
               'verification_method': None, 'failure_count': 0, 'http_status': None,
               'checked_url': None, 'retry_after': None, 'note': ''}
        try:
        ...
        if provider['network'] == 'tor':
            row.update(health='tor-unchecked', note='Tor probing is not supported; no request made.')
        elif provider['status'] == 'deprecated':
            row.update(health='deprecated', note='Deprecated definition; no request made.')
        elif not provider['enabled'] or provider['requires_api_key'] or provider['auth'] == 'required':
            row.update(health='manual-verification-required', note='Disabled, keyed or authenticated definition; no request made.')
        else:
            ...
            if host in cache:
                ...
                outcome = {'last_checked': datetime.now(timezone.utc).isoformat(),
                           'checked_url': endpoint, 'verification_method': 'HEAD origin homepage',
                ...
                try:
                    ...
                    outcome.update(health=interpret_status(status), reachable=True, http_status=status,
                    ...
                    outcome.update(health='manual-verification-required', reachable=None, note=str(exc))
