...
class CaseTests(unittest.TestCase):
    ...
    def test_persistence_deduplication_and_runs(self):
        ...
        with CaseStore(self.root) as store:
            ...
            self.assertEqual(data['relationships'][0]['provenance']['method'], 'email syntax parsing')
    ...
    def test_transaction_rollback_and_case_isolation(self):
        with CaseStore(self.root, writable=True) as store:
            ...
            relation = Relationship.create(plan.entity, 'references', stranger,
    ...
    def test_future_schema_and_foreign_database(self):
        ...
        self.assertEqual(db.execute('SELECT name FROM cases').fetchone()[0], 'preserved')
    ...
    def test_cli(self):
        with patch.dict(os.environ, {'D0RKW3B_DATA_DIR': str(self.root)}):
            for args in [('case', 'new', 'test-case'), ('case', 'add', 'test-case', 'example.com'),
                         ('case', 'note', 'test-case', 'Check history'),
                         ('investigate', 'alice@example.com', '--case', 'test-case', '--format', 'json'),
                         ('case', 'show', 'test-case'), ('case', 'list')]:
                out = io.StringIO()
