...
class ConfigurationTests(unittest.TestCase):
                ...
                json.dumps(
                    {
                        ...
                        "disabled_connectors": ["dns"],
                    }
                )
            )
            with patch.dict(os.environ, {"D0RKW3B_CONFIG": str(path)}):
                self.assertFalse(
                    next(p for p in load_registry()[0] if p["id"] == id)["enabled"]
                )
    ...
    def test_debug_and_invalid_config_keep_stdout_clean(self):
        ...
        with contextlib.redirect_stdout(out), contextlib.redirect_stderr(err):
            self.assertEqual(
                main(["ip", "bad", "--format", "json", "--debug", "--verbose"]), 2
            )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch.dict(os.environ, {"D0RKW3B_CONFIG": str(path)}),
