...
class CoreTests(unittest.TestCase):
    ...
    def test_inventory_coverage(self):
        ...
        self.assertEqual(
            {p["id"] for p in self.providers}, {p["id"] for p in inventory}
        )
        ...
        for provider in self.providers:
            ...
            variable = {"search": "query", "document": "query", "ipv4": "ip"}.get(
                kind, kind
            )
            ...
            rendered = render(
                provider,
                target,
                {k: samples[k] for k in provider["parameters"] if k != variable},
            )
            old = by_id[provider["id"]]["legacy_template"]
            for key, value in samples.items():
                old = old.replace(
                    "MIGRATION_" + key.upper() + "END", quote(value, safe="")
                )
            if not any(
                "Removed copied" in note or "Exclusive end" in note
                for note in by_id[provider["id"]]["changes"]
            ):
    ...
    def test_detection(self):
        ...
        self.assertEqual(
            validate("例え.テスト", "domain").value, "xn--r8jz45g.xn--zckzah"
        )
    ...
    def test_encoding(self):
        provider = dict(
            self.providers[0],
            template="https://example.com/{query}?q={query}#x={query}",
        )
        for value in [
            "a b",
            "a+b",
            ...
            self.assertEqual(
                result, f"https://example.com/{encoded}?q={encoded}#x={encoded}"
            )
    ...
    def test_schema_and_graceful_loading(self):
        ...
        for change in [
            ...
            {"template": "https://a.com/{bad}"},
            {"template": "https://a.com/{query"},
            {"template": "https://a.com/{query!r}"},
            {"target_types": ["bogus"]},
            {"enabled": "yes"},
            {"id": "BAD"},
            {"network": "tor"},
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(
                json.dumps([valid, {"id": "missing"}, valid | {"id": "community-test"}])
            )
            providers, errors = load_registry([path])
    ...
    def test_parameters_and_browser(self):
        year = next(
            p for p in self.providers if p["source"] == "x.py:x_username_by_year"
        )
        result = self.invoke(
            "username", "someone", "--provider", year["id"], "--param", "year=2024"
        )
        self.assertEqual(result[0], 0)
        self.assertIn("until%3A2025-01-01", result[1])
        self.assertEqual(
            self.invoke("username", "someone", "--provider", year["id"])[0], 2
        )
        with patch("webbrowser.open", return_value=True) as browser:
            ...
            clear = next(
                ...
                if p["source"] == "search_links.py:generate_links"
                and p["network"] == "clearnet"
            )
            self.assertEqual(
                self.invoke("search", "hello", "--provider", clear["id"], "--open")[0],
                0,
            )
    ...
    def test_exports_and_entrypoint(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "d0rkw3b", "--version"],
            capture_output=True,
            text=True,
        )
...
class InteractionTests(unittest.TestCase):
    def test_interactive_recovery_and_exit(self):
        ...
        with (
            patch(
                "builtins.input",
                side_effect=["help", "domain example.com", '"bad', "quit"],
            ),
    ...
    def test_guided_menu(self):
        ...
        out = io.StringIO()
        with (
            patch("builtins.input", side_effect=["1", "example.com"]),
            contextlib.redirect_stdout(out),
        ):
    ...
    def test_ipv6_and_disabled_reference(self):
        ...
        with contextlib.redirect_stdout(out):
            self.assertEqual(
                main(["ip", "2001:4860:4860::8888", "--format", "json"]), 0
            )
        ...
        providers, _ = load_registry()
        self.assertTrue(
            all(not p["enabled"] for p in providers if p["requires_api_key"])
        )
