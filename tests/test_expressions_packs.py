...
class ExpressionPackTests(unittest.TestCase):
    def test_small_grammar(self):
        self.assertEqual(parse_expression('domain:example.com category:domain'), ('domain', 'example.com', 'domain'))
        self.assertEqual(parse_expression('search:"Acme Corporation"'), ('search', 'Acme Corporation', None))
        self.assertEqual(parse_expression('ip:2001:4860:4860::8888')[1], '2001:4860:4860::8888')
        for bad in ['eval:1+1', 'domain:example.com AND category:archive', 'domain:example.com sort:date']:
            with self.assertRaises(ValueError):
                parse_expression(bad)
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            self.assertEqual(main(['investigate', 'domain:example.com category:domain', '--expression']), 0)
    ...
    def test_pack_counts_and_offline_filter(self):
        ...
        with contextlib.redirect_stdout(out):
            self.assertEqual(main(['providers', 'list', '--pack', 'development', '--format', 'json']), 0)
