...
class EntityTests(unittest.TestCase):
    def test_normalization_identity(self):
        self.assertEqual(Entity.create('Example.COM', 'domain').entity_id,
                         Entity.create('example.com', 'domain').entity_id)
        self.assertEqual(Entity.create('@alice').entity_id, Entity.create('alice', 'username').entity_id)
        self.assertNotEqual(Entity.create('@alice').entity_id, Entity.create('alice@example.com').entity_id)
        self.assertNotEqual(Entity.create('Alice', 'username').entity_id, Entity.create('alice', 'username').entity_id)
    ...
    def test_provenance_required(self):
        ...
        with self.assertRaises(ValueError):
            Observation('id', 'generated_query', 'test', 'render', entity.entity_id, STAMP,
                        provenance, output_entity_id='not-a-finding')


class InvestigationTests(unittest.TestCase):
    def test_api_offline_and_stable_with_clock(self):
        with patch('socket.socket', side_effect=AssertionError('network forbidden')):
            plan = investigate('example.com', recipe='domain-footprint', created_at=STAMP)
            self.assertEqual(plan.to_dict(), investigate('example.com', recipe='domain-footprint', created_at=STAMP).to_dict())
    ...
    def test_recipes(self):
        ...
        for recipe in recipes:
            ...
            sample = {'domain': 'example.com', 'username': 'alice', 'email': 'alice@example.com',
                      'repository': 'repo', 'document': 'annual report', 'url': 'https://example.com/image.png', 'company': 'acme'}[kind]
            self.assertTrue(investigate(sample, type=kind, recipe=recipe['id']).paths)
        broken = dict(recipes[0], stages=[{'name': 'bad', 'description': 'bad', 'providers': ['invented']}])
        with self.assertRaises(ValueError):
    ...
    def test_cli_groups(self):
        for args in [('recipes', 'list'), ('recipes', 'info', 'domain-footprint'),
                     ('dev', 'validate-recipes'), ('investigate', 'example.com'),
        ...
        for fmt in ('json', 'csv', 'markdown'):
            ...
            with contextlib.redirect_stdout(out), contextlib.redirect_stderr(io.StringIO()):
                self.assertEqual(main(['investigate', 'example.com', '--format', fmt]), 0)
            if fmt == 'json':
