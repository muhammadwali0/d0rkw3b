...
class EvidenceTests(unittest.TestCase):
    def test_import_preserves_bytes_and_provenance(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with CaseStore(root / 'data', writable=True) as store:
                ...
                self.assertEqual(saved['evidence'][0]['provenance']['method'], 'byte-preserving import and SHA-256')
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            if os.name != 'nt':
                with CaseStore(root / 'data', writable=True) as store:
                    ...
                    (store.root / 'evidence' / case['case_id']).symlink_to(root, target_is_directory=True)
    ...
    def test_local_metadata_and_references(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text('Contact alice@example.com and visit https://example.com', encoding='utf-8')
            result = inspect_file(path)
