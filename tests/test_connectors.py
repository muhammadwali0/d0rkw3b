...
class ConnectorTests(unittest.TestCase):
    def test_dns_observed_relationships_and_no_shell(self):
        ...
        with patch('d0rkw3b.connectors.builtin.run_json', return_value=['1.1.1.1', '2606:4700:4700::1111']) as process:
            result = dns(entity)
        result.validate()
    ...
    def test_rdap_bootstrap_and_object_validation(self):
        ...
        with patch('d0rkw3b.connectors.builtin.request', side_effect=[
            (200, {}, json.dumps(bootstrap).encode()), (200, {}, json.dumps(data).encode())]) as network:
            result = rdap(entity)
        self.assertEqual(network.call_args_list[0].args[0], 'https://data.iana.org/rdap/dns.json')
        self.assertEqual(network.call_args_list[1].args[0], 'https://rdap.registry.example/v1/domain/example.com')
        self.assertEqual(result.relationships[0].predicate, 'uses_nameserver')
        self.assertEqual(result.entities[1].type, 'hostname')
        with patch('d0rkw3b.connectors.builtin.get_json', side_effect=[bootstrap, data | {'ldhName': 'different.com'}]):
    ...
    def test_rdap_ip_longest_prefix(self):
        bootstrap = {'services': [[['0.0.0.0/0'], ['https://general.example/']],
                                  [['1.0.0.0/8'], ['https://specific.example/']]]}
        self.assertEqual(rdap_base(bootstrap, Entity.create('1.1.1.1')), 'https://specific.example/')
    ...
    def test_local_file_and_explicit_dispatch(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch('socket.socket', side_effect=AssertionError('network forbidden')):
                ...
                self.assertTrue(disclosure.call_args.args[0].capabilities.filesystem_read)
                out, err = io.StringIO(), io.StringIO()
                with contextlib.redirect_stdout(out), contextlib.redirect_stderr(err):
                    self.assertEqual(main(['file', str(path)]), 0)
                self.assertIn('Capabilities', err.getvalue())
                self.assertEqual(json.loads(out.getvalue())['observations'][0]['kind'], 'observed')
    ...
    def test_missing_tools_and_exiftool_read_only_options(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch('shutil.which', return_value='/usr/bin/exiftool'), patch(
                'd0rkw3b.connectors.integrations.run_json', return_value=[{'EXIF:Make': 'Example'}]) as tool:
    ...
    def test_bounded_process(self):
        self.assertEqual(run_json([sys.executable, '-I', '-c', 'print("{\\\"ok\\\": true}")']), {'ok': True})
        with self.assertRaisesRegex(ConnectorUnavailableError, 'timed out'):
            run_json([sys.executable, '-I', '-c', 'import time; time.sleep(5)'], timeout=0.1)
    ...
    def test_investigate_does_not_import_plugins_implicitly(self):
        with patch('d0rkw3b.connectors.registry.entry_points', side_effect=AssertionError('plugin discovery')):
            out = io.StringIO()
            with contextlib.redirect_stdout(out), contextlib.redirect_stderr(io.StringIO()):
                self.assertEqual(main(['investigate', 'example.com', '--format', 'json']), 0)
...
class PluginTests(unittest.TestCase):
    def test_static_discovery_and_explicit_loading(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(json.dumps({'api_version': 1, 'connectors': {'test-plugin': manifest}}))
            entry = Mock()
            entry.name = 'test-plugin'
            entry.dist.files = [path]
            entry.dist.locate_file.return_value = path
            runner = Mock(side_effect=lambda entity, timeout: Collection(entity, [entity], [], []))
