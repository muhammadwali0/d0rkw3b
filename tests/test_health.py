...
class HealthTests(unittest.TestCase):
    def setUp(self):
        ...
        self.provider = next(
            p for p in self.providers if p["enabled"] and p["network"] == "clearnet"
        )
    ...
    def test_status_interpretation(self):
        for status, expected in [
            ...
            (301, "redirect"),
            (401, "auth-required"),
            (403, "manual-verification-required"),
            (405, "manual-verification-required"),
            (429, "rate-limited"),
            (404, "degraded"),
            (503, "degraded"),
        ]:
    ...
    def test_no_semantic_claim_and_no_targets(self):
        ...
        provider = self.provider | {
            "homepage": "https://example.com/private?target=SECRET"
        }
    ...
    def test_bounds_rate_limits_and_skips(self):
        ...
        result = check_providers(
            self.providers, max_requests=2, probe=probe, sleep=sleep
        )
    ...
    def test_network_errors(self):
        for error, status in [
            ...
            row = check_providers([self.provider], probe=Mock(side_effect=error))[
                "results"
            ][0]
